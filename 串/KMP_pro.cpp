...
//通过计算返回子串T的nextval数组，时间复杂度O(m)，m是子串T的长度
	...
	while(i<T.length)
	{
		if(j==0||T.data[i]==T.data[j])//前缀字符下标j一直回溯到0，重新开始匹配
		{
			...
			if(T.data[i]!=T.data[j])
			    nextval[i]=j;  //计算意义：表示后缀字符T.data[i]与前缀字符T.data[j]元素相等，则i+1如果不匹配的话就可以通过nextval[i+1]回溯到j+1进行比较
			                //逻辑意义：表示后缀字符T.data[i]与前缀字符T.data[j]元素相等，nextval[i+1]存储的就是前i的子串里面有j个字符相等，可以回溯到j+1进行比较，因为j前面都是相等的
			else
				nextval[i]=nextval[j]; /*如果前缀下一个字符和后缀下一个字符相等，则两个字符的nextval值相等，因为这个字符不匹配，回溯后的字符也不匹配，这样一步回溯到位，不用回溯好几次才回溯完，省去多余的重复计算*/
		}
		else
			j=nextval[j];  //表示后缀字符T.data[i]与前缀字符T.data[j]元素不相等，则前缀字符下标j直接回溯到nextval[j]，回溯到1
	}
}
...
int Index_KMP(STR S,STR T,int pos)
{
	...
	int nextval[255];    //定义nextval数组
	get_nextval(T,nextval);    //计算子串T的nextval数组
	...
	{                                  //直到j加到数值大于T的长度匹配成功，或者i加到数值大于S的长度结束匹配这种情况可能匹配不成功可能匹配成功，若成功则T是S的后缀
		...
		{                                 //或者当子串T与主串S匹配到了，i++和j++继续匹配
			...
			j=nextval[j];    //当前位置的T元素和S元素不匹配，j回溯到前j个字符串中前缀与后缀匹配相等时前缀的后一个字符下标的位置，i不变
	}
	if(j>T.length)
